...
import dev.slne.surf.eventbus.config.settings.EventBusSettings
...
private fun resolveRabbitMQ(
    global: EventBusRabbitMQConfig?,
    plugin: EventBusRabbitMQConfig?,
    environment: EnvironmentVariables,
) = RabbitMQSettings(
    host =
        environment.optional(RABBIT_PREFIX + "HOST") {
            require("expected a non-blank host") { it.isNotBlank() }
        } ?: yaml(plugin?.host?.value, global?.host?.value) ?: EventBusDefaults.RABBIT_HOST,
    port =
        environment.optionalInt(RABBIT_PREFIX + "PORT") { requireIn(1..65535) }
            ?: yaml(plugin?.port?.value, global?.port?.value) ?: EventBusDefaults.RABBIT_PORT,
    username =
        environment.optional(RABBIT_PREFIX + "USERNAME") {
    ...
    password =
        environment.optional(RABBIT_PREFIX + "PASSWORD", sensitive = true)
            ?: yaml(plugin?.password?.value, global?.password?.value)
            ?: EventBusDefaults.RABBIT_PASSWORD,
    vhost =
        environment.optional(RABBIT_PREFIX + "VHOST") {
            require("expected a non-blank vhost") { it.isNotBlank() }
        } ?: yaml(plugin?.vhost?.value, global?.vhost?.value) ?: EventBusDefaults.RABBIT_VHOST,
    timeout =
        environment.optionalInt(RABBIT_PREFIX + "TIMEOUT") {
    ...
    requestTimeoutSeconds =
        environment.optionalInt(RABBIT_PREFIX + "REQUEST_TIMEOUT_SECONDS") {
            require("expected a positive timeout") { it > 0 }
        } ?: yaml(plugin?.requestTimeoutSeconds?.value, global?.requestTimeoutSeconds?.value)
            ?: EventBusDefaults.REQUEST_TIMEOUT_SECONDS,
    publisherPoolSize =
        environment.optionalInt(RABBIT_PREFIX + "PUBLISHER_POOL_SIZE") {
    ...
    serverPrefetchCount =
        environment.optionalInt(RABBIT_PREFIX + "SERVER_PREFETCH_COUNT") {
    ...
    persistRequests =
        environment.optionalBoolean(RABBIT_PREFIX + "PERSIST_REQUESTS")
            ?: yaml(plugin?.persistRequests?.value, global?.persistRequests?.value)
            ?: EventBusDefaults.PERSIST_REQUESTS,
    persistResponses =
        environment.optionalBoolean(RABBIT_PREFIX + "PERSIST_RESPONSES")
    ...
    auditServiceName =
        environment.optional(AUDIT_SERVICE_VARIABLE) {
...
private fun resolveRedis(
    global: EventBusRedisConfig?,
    plugin: EventBusRedisConfig?,
    environment: EnvironmentVariables,
) = RedisSettings(
    host =
        environment.optional(REDIS_PREFIX + "HOST") {
            require("expected a non-blank host") { it.isNotBlank() }
        } ?: yaml(plugin?.host?.value, global?.host?.value) ?: EventBusDefaults.REDIS_HOST,
    port =
        environment.optionalInt(REDIS_PREFIX + "PORT") { requireIn(1..65535) }
            ?: yaml(plugin?.port?.value, global?.port?.value) ?: EventBusDefaults.REDIS_PORT,
    password =
        environment.optional(REDIS_PREFIX + "PASSWORD", sensitive = true)
            ?: yaml(plugin?.password?.value, global?.password?.value),
    clientName =
        environment.optional(REDIS_PREFIX + "CLIENT_NAME") {
