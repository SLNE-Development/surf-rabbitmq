 ...
 *
 * ## Usage
 * A common setup is to provide a single, platform-owned [SurfRedisApi] instance via a service and expose
 ...
 */
@Suppress("unused")
class SurfRedisApi private constructor(
    config: Config,
    /** JSON instance used internally for (de-)serialization. */
    val json: Json,
) : EventBusTransporter(ConfigSupport.getConfig(config).clientName) {
    override val connection = RedisConnection(config)

    /**
     * Underlying Redisson client. Initialized by [connect]; accessing it before then fails.
     *
    ...
    /**
     * Whether the Redis client is initialized and not shutting down.
     *
     * This does not guarantee Redis availability; it only reflects the local client state. Use
     * [isAlive] for an active check.
     */
    val isConnected get() = connection.isConnected

    /**
     * Performs an active health check against Redis by sending a `PING`.
     *
     * @return `true` if Redis responds successfully, `false` otherwise.
     */
    suspend fun isAlive(): Boolean = connection.isAlive()

    companion object {
        /**
         * Builds a [SurfRedisApi] that honours all four config layers.
         *
         * The route to prefer over the `create(...)` overloads: [dataPath] is what the plugin
         * layer is read from, and none of the overloads can express it.
         *
         * @param pluginName Logical name of the calling plugin or component.
         * @param dataPath the consumer's own data folder. `null` for a caller with no folder of
         *   its own, which resolves `env > global > default`.
         */
        fun builder(
            pluginName: String,
            dataPath: Path? = null,
        ): SurfRedisApiBuilder = SurfRedisApiBuilder(pluginName, dataPath)

        /**
         * Creates a [SurfRedisApi] instance using the given [redisURI].
         *
        ...
        /**
         * Creates a [SurfRedisApi] from already-resolved four-layer [settings].
         *
        ...
        fun create(
            ...
            create(
                settings = settings,
                redisURI =
                    RedisCredentialsProvider.redisURI(
                        RedisCredentialsProvider.credentials(settings),
                    ),
                pluginName = pluginName,
                serializerModule = serializerModule,
            )
        ...
        private fun create(
            ...
            val config =
                RedisComponentProvider.createRedissonConfig(
            ...
            return SurfRedisApi(config, createJson(serializerModule))
        }
        ...
        fun create(
            redisURI: RedisURI,
            serializerModule: SerializersModule = EmptySerializersModule(),
        ): SurfRedisApi = create(redisURI, getCallingPluginName(), serializerModule)

        /**
         * Creates a [SurfRedisApi] for an address such as `redis://localhost:6379`.
         *
        ...
        fun createForAddress(
            address: String,
            serializerModule: SerializersModule = EmptySerializersModule(),
        ): SurfRedisApi = create(RedisURI(address), getCallingPluginName(), serializerModule)

        /**
         * Creates a [SurfRedisApi] instance using credentials provided by
         ...
         * @param serializerModule Additional serializers to be included in the internal [Json] instance.
         */
        fun create(serializerModule: SerializersModule = EmptySerializersModule()): SurfRedisApi =
            create(
                RedisCredentialsProvider.settings(null),
                getCallingPluginName(),
                serializerModule,
            )

        /**
         * Creates a [SurfRedisApi] instance using the default Redis credentials.
         *
         ...
         * @param pluginName Logical name of the calling plugin or component.
         */
        fun create(pluginName: String): SurfRedisApi = create(RedisCredentialsProvider.settings(null), pluginName, EmptySerializersModule())
        ...
        fun create(
            pluginName: String,
            serializerModule: SerializersModule,
        ): SurfRedisApi = create(RedisCredentialsProvider.settings(null), pluginName, serializerModule)
        ...
        private fun createJson(serializerModule: SerializersModule) =
            Json {
    ...
    private inline fun <S : SyncStructure<*>> createSyncStructure(creator: () -> S): S {
        ...
        connection.registerInitializable(structure)
        connection.registerDisposable(structure)
        return structure
    }
    ...
    fun <K : Any, V : Any> createSimpleCache(
        ...
        if (isConnected) {
            ...
            runBlocking { connection.initialize(cache) }
        } else {
            connection.registerInitializable(cache)
        }

        connection.registerDisposable(cache)
        return cache
    }
